...
- The installer's instructions now suit a box with no `sudo`, using `su` or dropping the privileged route entirely.
- The apt and dnf recipes are a single command, so a partial paste can no longer install an older distro Node.
- The installer no longer rejects an npm prefix that is configured but not yet created.
- Node without npm, the shape Debian's own package leaves you in, is explained instead of being called a broken install.
- The nvm route says to run it as the user that will run the node, not as root.
