...
- Repository setup no longer blocks the node while it runs, so a long install cannot make a busy node look unresponsive.
- A node now declines a remote update while it is running stages, instead of installing one it cannot restart into.

### Added

- A node's heartbeat now reports which stages it is executing, so a brief disconnection no longer risks a running stage.
